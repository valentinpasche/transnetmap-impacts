...
class Results:
    """
    **Base class for computing and managing environmental and energy impacts in a transport network.**
    ...
    The class is **not meant to be used directly** but serves as a **parent class** for specialized implementations such as `HeatMap()`.  
    ...
    **Checking if a partial network already exists**
    ...
    def prepare_partial_network(self, id_zone: int) -> None:
        ...
        table_exists, columns_complete = self._validate_existing_partial_network(self.impacts_statut["current"])
        ...
        if table_exists and not columns_complete:
            ...
            self._log(f"⚠️ The existing table `results_{id_zone}` has missing impact columns. "
    ...
    def replace_all_impacts_in_db(self) -> None:
        ...
        if self._id_valid is not None:
            raise RuntimeError("❌ The `Results` instance must not be linked to a specific `id_zone` before running this method.")
    
        if self.table is not None or self.optimisation_updated_table is not None:
            raise RuntimeError("❌ The `Results` instance must be clean. Ensure that `self.table` and `self.optimisation_updated_table` are `None` before execution.")
    
        # 🔹 Step 2: Confirm user intention
        confirmation = input(
            "⚠️ This operation will replace impact columns in the database for all tables, including edgelist and results_{id_zone}.\n"
        ...
        if edgelist_table.is_empty():
            raise RuntimeError("❌ The edgelist table is empty. Ensure that impacts have been calculated correctly.")
    ...
    #%% Validation Methods
    # ===========================
    def _validate_existing_partial_network(self, impact_columns: list) -> tuple[bool, bool]:
        """
        ...
        existing_columns = columns_exist(self.uri, required_columns, table_name, schema, print_status=self.main_print)
    
        missing_columns = [col for col, exists in existing_columns.items() if not exists]
        if missing_columns:
            self._log(f"⚠️ Table '{table_name}' is missing columns: {', '.join(missing_columns)}. A recalculation is required.")
            return True, False  # ⚠️ Table exists but missing impacts
    ...
    # ===========================
    def _load_impacts(self, impacts_list=False) -> "Results":
        """
        ...
        impacts_list : list of str, optional
            A list of impact types to load (e.g., ["CO2", "EP"]). If not provided, all available
        ...
        from transnetmap.utils.dct import impacts_list as impacts_list_default
        from transnetmap.utils.sql import table_exists
    
        # 🔹 Step 1: Reset impact instances and tracking
        self.dct_impacts_instances = {}
        self.impacts_statut = {"default": sorted(impacts_list_default), "loaded": [], "missing": []}
    
        # 🔹 Step 2: Validate the provided impacts_list
        if impacts_list:
            invalid_impacts = [impact for impact in impacts_list if impact not in impacts_list_default]
            if invalid_impacts:
                raise RuntimeError(
                    f"Invalid impacts provided: {', '.join(invalid_impacts)}.\n"
                    f"Allowed impacts: {', '.join(impacts_list_default)}."
                )
        else:
            impacts_list = impacts_list_default
        ...
        if self.main_print:
            ...
            if self.impacts_statut["missing"]:
                print(f"⚠️ Missing impact tables: {', '.join(self.impacts_statut['missing'])}")
    ...
    def _determine_impacts_to_update(self, force_replace: bool = False) -> bool:
        ...
        if not table_exists(self.uri, table_name_optimisation, print_status=self.main_print):
            raise RuntimeError(f"❌ Table '{schema}.{table_name_optimisation}' does not exist.\n"
                               "Ensure the new network has been optimised and recorded before post-processing the results.")
    
        # 🔹 Step 2: Load impact tables
        self._load_impacts()
    
        if not self.impacts_statut["loaded"]:
            raise RuntimeError("❌ No impact tables were successfully loaded. Cannot proceed with update determination.")
        ...
        columns_exist_in_db = columns_exist(self.uri, impacts_list_default, table_name_edgelist, schema, print_status=self.main_print)
        ...
        self.impacts_statut["current"] = sorted([col for col, exists in columns_exist_in_db.items() if exists])
        
        # 🔹 Step 4: Identify suppressed impacts
        inconsistent_columns = [col for col in self.impacts_statut["current"] if col not in self.impacts_statut["loaded"]]
        self.impacts_statut["inconsistent"] = inconsistent_columns
        
        if inconsistent_columns and not force_replace:
            raise RuntimeError(f"❌ The following impacts exist in 'edgelist' but have no corresponding database table: {', '.join(inconsistent_columns)}.\n"
                               "Execute `replace_all_impacts_in_db()` to reset all impact tables and restore consistency."
        ...
        if not columns_to_update:
            self._log(f"✅ No impacts need to be updated. Current impacts in DB: {', '.join(self.impacts_statut['current'])}")
    ...
    def _update_edgelist_in_db(self, force_replace: bool = False) -> dict:
        ...
        This method calculates and updates impact values (e.g., CO2, EP) for the 'edgelist' table.
        ...
        valid_columns = ["from", "to", "type", "time", "length"] + [
        ...
        for col in check_invalid.columns:
            if not check_invalid.select(col).filter(pl.col(col) > 0).is_empty():
                raise RuntimeError("❌ The table used to calculate impacts contains inconsistent values (polars.Null).")
    
        # 🔹 Step 4: Calculate missing impact values (returns only new impact columns)
        self._log("🛠️ Calculating missing impact values...")
        ...
        updated_edgelist = updated_edgelist.select(["from", "to", "type", "time", "length", *current_impacts]).with_columns(
    ...
    # ===========================    
    def _add_impacts_on_edge(self, table: pl.DataFrame, columns_to_update: list) -> pl.DataFrame:
        """
        Adds calculated impact values (e.g., CO2, EP) to a given edgelist table (e.g., edgelist, NPTM tables).
        ...
        for impact_name in columns_to_update:
            ...
            for _type in working_table["type"].unique():
                ...
                impact_filtered = impact_pl.filter(pl.col("type") == _type).sort("max_distance", nulls_last=True)
                
                # ⚠️ Sorting must be done last to satisfy `join_asof()` requirements in Polars
                table_with_impact = table_filtered.join_asof(
                    ...
                    .then(impact_filtered.filter(pl.col("max_distance").is_null()).select("impact_value").item(0, 0))
        ...
        if missing_impacts:
            raise RuntimeError(f"❌ The following impact columns are missing in the final table: {', '.join(missing_impacts)}")
    
        # Validate that no null values exist in the final table
        check_invalid = table.null_count()
        for col in check_invalid.columns:
            if not check_invalid.select(col).filter(pl.col(col) > 0).is_empty():
                raise RuntimeError("❌ The table resulting from the impact calculation contains inconsistent values (polars.Null).")
    
        self._log(f"✅ Impacts successfully added: {', '.join(columns_to_update)}")
        return table


    def _add_impacts_on_optimisation(self, optimisation_table: pl.DataFrame, edgelist_table: pl.DataFrame, current_impacts: list) -> pl.DataFrame:
        """
        Calculates impacts and length for the partial 'optimisation' table.
    
        This method computes impact values (e.g., CO2, EP) and length for the 'optimisation' table 
        ...
        for col in check_invalid.columns:
            if not check_invalid.select(col).filter(pl.col(col) > 0).is_empty():
                raise RuntimeError("❌ The final optimisation table contains inconsistent values (polars.Null).")
    ...
    def _process_partial_network(self, id_zone: int, edgelist_table: pl.DataFrame, current_impacts: list) -> pl.DataFrame:
        """
        ...
        def __process_nptm_table(_type: str, id_zone: int, columns_to_add: list) -> pl.DataFrame:
            ...
            for col in check_invalid.columns:
                if not check_invalid.select(col).filter(pl.col(col) > 0).is_empty():
                    raise RuntimeError("❌ NPTM impact calculation resulted in inconsistent values (polars.Null).")
        ...
        optimisation_table = self._add_impacts_on_optimisation(optimisation_table, edgelist_table, current_impacts)
    ...
    def read_sql_partial_network(self, id_zone: int = None, columns=None, where_condition=None) -> pl.DataFrame:
        """
...
if __name__ == "__main__":
    ...
    Network(dct_param).read_sql().add_stations().show(two_map=True)
    Network(dct_param).show_all()
