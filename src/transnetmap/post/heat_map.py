...
class HeatMap(Results):
    ...
    **Main interface for generating interactive heatmaps based on a transport network.**
    ...
    ⚠️ The class does **not** include a `.show()` method like `Network()` or `Stations()`.
    ...
    **Parameters (via __init__)**
    ...
    network_config : dict or ParamConfig
        ...
        Required fields (if using a dictionary):
    ...
    **Public Method**
    ------------------
    generate_map(...)
        Generates and saves the interactive map based on selected layers.

    **Helper Methods**
    ------------------
    show_layers()
        Displays the list of valid choropleth layers (with categories).
        
    show_popup_fields()
        Displays valid popup fields based on database columns.
        
    info()
        Displays general settings for the current HeatMap instance.
    ...
    **Attributes**
    ...
    impacts : list of str
        List of environmental impact types available for display (e.g., "EP", "CO2").
    ...
    **Example**
    -----------
    ```python
    from transnetmap.post.heat_map import HeatMap
    ...
    #%% Constructor and Execution Context
    # ===========================
    def __init__(
        self,
        ...
        map_config : HeatMapConfig, optional
            Custom configuration for heat map generation. If None, uses default.
        ...
        self.direction = {"primary": "from", "secondary": "to"} if from_zone else {"primary": "to", "secondary": "from"}
    ...
    def _cleanup_results_attributes(self):
        """
    ...
    # 🔹 Step 0: Generic methods
    def _log(self, message: str) -> None:
        """Handles conditional logging based on the `main_print` flag."""
        if self.main_print:
            print(message)

    def replace_all_impacts_in_db(self):
        raise NotImplementedError("This method cannot be called from `HeatMap`. It is reserved for `Results`.")
    ...
    def _initialize_heatmap_layers(self):
        """
        ...
        The generated dictionary is stored in `self.heatmap_layers`.
    
        Returns
        -------
        dict
            The dynamically constructed heatmap layers dictionary.
        ...
        heatmap_layers["type_NTS"] = {
            ...
            "legend_name": f"Route transport type ({dct_type['with-NTS']} = NTS, {dct_type['extend-NTS']} = {self.network_extension_type})",
        }
    ...
    def show_layers(self):
        ...
        print("\nℹ️  Use `self.heatmap_layers` to access metadata.")

    def show_popup_fields(self):
        """
        ...
        print("\nℹ️  Use `self.columns_metadata` for details.")
    ...
    def generate_map(
        ...
        *,
        ...
        file_name : str, optional
            The base filename (without extension) for saving the HTML map. Defaults to `"heatmap"` via config.
        
        save_to_desktop : bool, optional
            If True, saves the HTML file directly to the user's desktop. Mutually exclusive with `custom_path`.
        ...
        include_network_layers : bool, optional
            If True, overlays the original network geometries (e.g., PT lines, IMT segments, NTS levels).
        ...
        self._log("🗺️ Creating base map...")
        m = auto_fit_map(self.geo_data, location=self.heatmap_config.location, zoom_start=self.heatmap_config.zoom_start, tiles=False)
        setup_tiles(m, map_tiles)
        ...
        if choropleths:
            ...
            for layer_name, layer_data in choropleth_layers.items():
                ...
                if analysis_type in unique_captions:
                    self._log(f"ℹ️ Custom legend for analysis type `{analysis_type}` already exist.")
                    pass
                else:
        ...
        Fullscreen(position="topleft", title="Full screen", title_cancel="Exit", force_separate_button=True).add_to(m)
        ...
        show_map(m, file_name=file_name, save_to_desktop=save_to_desktop, custom_path=custom_path, open_browser=open_browser)
    ...
    def _validate_choropleths_popup_name(
        ...
        if choropleths == [] or choropleths is None:
            choropleths_layers = []
            self._log("ℹ️ No choropleth layers requested. Generating a base map only.")
        elif isinstance(choropleths, list):
            if not all(isinstance(layer, str) for layer in choropleths):
                raise ValueError("❌ `choropleths` list must contain only strings (keys of `self.heatmap_layers`).")
            ...
            if invalid_layers:
                raise ValueError(f"❌ Invalid choropleths requested: {invalid_layers}. Must match `self.heatmap_layers` keys.")
        else:
            raise ValueError(f"❌ Invalid type for `choropleths`: {type(choropleths)}. Must be list or None.")
        ...
        if popup is False:
            ...
            self._log("ℹ️ No popup fields defined. Using same fields as choropleths (from `depends_on`).")
        elif isinstance(popup, list):
            if not all(isinstance(field, str) for field in popup):
                raise ValueError("❌ `popup` list must contain only strings (columns names of `self.table`).")
            popup_fields = popup
            invalid_fields = [field for field in popup_fields if field not in self.table.columns]
            if invalid_fields:
                raise ValueError(f"❌ Invalid popup requested: {invalid_fields}. Must match `self.table.columns`.")
            elif popup_fields == []:
                self._log("ℹ️ Popups list is empty (no fields selected).")
            else:
    ...
    def _warn_if_map_too_heavy(self, n_layers: int) -> Optional[str]:
        ...
        - ≥ 300 MB or ≥ 5 layers → ⚠️ Soft warning (slow load likely)
        ...
        if n_layers >= 8 or estimated_size_mb > 500:
            warning_log = ("\n"
                ...
                "⚠️  This map may fail to load in some browsers (e.g. Chrome).\n"
                "💡 Recommendation: Split into multiple maps.\n"
            )
        elif n_layers >= 5 or estimated_size_mb > 300:
            warning_log = ("\n"
                f"⚠️  Large map detected (~{estimated_size_mb} MB, {n_layers} layers).\n"
    ...
    def _load_heatmap_data(self) -> gpd.GeoDataFrame:
        ...
        NTS_table = results_table[results_table["type"].isin([dct_type["with-NTS"], dct_type["extend-NTS"]])].add_prefix("NTS_")
        IMT_table = results_table[results_table["type"] == dct_type["IMT"]].drop(columns=["type", "nb_edges"]).add_prefix("IMT_")
        PT_table = results_table[results_table["type"] == dct_type["PT"]].drop(columns=["type", "nb_edges"]).add_prefix("PT_")
    ...
    def _prepare_difference_layer(self, layer_name: str) -> pd.DataFrame:
        ...
        if layer_name not in self.heatmap_layers:
            raise ValueError(f"❌ The requested layer `{layer_name}` is not in `self.heatmap_layers` keys.")
        ...
        if len(depends_on) != 2:
            raise ValueError(f"❌ `{layer_name}` must have exactly 2 dependencies in `depends_on`. Found: {depends_on}")
    ...
    def _prepare_layers(self, choropleths: List[str]) -> Dict[str, Dict[str, Any]]:
        ...
        """
        self._log("🛠️ Setup choropleth layers data...")
        ...
        if invalid_layers:
            raise ValueError(f"❌ Invalid choropleths requested: {invalid_layers}. Check `self.heatmap_layers` keys for valid options.")
        ...
        for layer in choropleths:
            ...
            if "depends_on" not in layer_info:
                ...
                self._log(f"ℹ️ Layer `{layer}` is ignored.")
            ...
            if len(layer_info["depends_on"]) == 1:
                ...
                # Standard single-column layers
                    ...
                    dct_layer["data"] = self.table[["id", column_name]].rename(columns={column_name: "values"})
    ...
    # ===========================
    def _get_scale_config(self, analysis_type: str) -> Dict[str, Union[List[float], float, float, str, bool]]:
        """
        ...
        """
        from transnetmap.utils.dct import defaults_thresholds_scale_color
        ...
        default_config = None
        if is_difference:
            default_config = defaults_thresholds_scale_color["difference"]
        elif is_impact:
            default_config = defaults_thresholds_scale_color["impacts"]
        elif analysis_type in defaults_thresholds_scale_color:
            default_config = defaults_thresholds_scale_color[analysis_type]
        else:
            raise KeyError(f"⚠️ Unknown analysis type `{analysis_type}`. No default values found.")
        ...
        for key in ["fill_color", "reverse_color"]:
            if self.heatmap_config.thresholds_scale[analysis_type][key] is None:
                raise KeyError(f"⚠️ `{key}` is missing for `{analysis_type}` in `thresholds_scale`.")
    
        return self.heatmap_config.thresholds_scale[analysis_type]


    def _compute_threshold_scale(self, analysis_type: str, data: pd.DataFrame, bins: int = 8) -> Tuple[str, List[float]]:
        """
        ...
        """
        if bins < 4:
            raise ValueError("⚠️ Number of threshold bins must be at least 4.")
        ...
        for col in data.columns:
            if data[col].isna().any():
                raise ValueError(f"⚠️ Column `{col}` contains NaN values.")

        all_values = data.to_numpy().flatten()

        if all_values.size == 0:
            raise ValueError("⚠️ No valid data found. Empty input.")
        ...
        # 🔹 Step 3: User-defined scale
            ...
            if scale is None:
                ...
                self._log(f"⚠️ Fallback to dynamic Jenks scale for analysis type `{analysis_type}`.")
    ...
    def _create_custom_branca_legend(self, analysis_type: str, layer_meta: dict) -> StepColormap:
        ...
        if not scale or not color_scale:
            raise ValueError(f"❌ Missing scale or color scale for analysistype `{analysis_type}`. Cannot create legend.")
        
        if len(scale) != len(color_scale) + 1:
            raise ValueError(
                f"❌ Mismatch in scale and color count for analysis type `{analysis_type}`. "
                f"Expected {len(scale) - 1} colors for {len(scale)} thresholds, got {len(color_scale)} colors."
            )
    ...
    def _remove_choropleth_legend(self, choropleth: folium.Choropleth) -> None:
        ...
        if keys_to_remove:
            ...
            self._log(f"ℹ️ No legend found to remove in layer `{choropleth.layer_name}`.")
    ...
    def _create_choropleth_layer(self, data: pd.DataFrame, layer_meta: dict) -> folium.Choropleth:
        ...
        """
        self._log(f"🖌️ Creating choropleth layer `{layer_meta['label']}`...")
        ...
        self._log("⚠️ Choropleth layer settings (overlay, control, show) have been forced for consistent display.")
        ...
        choropleth = folium.Choropleth(
            ...
            data=data, # Data to bind to the GeoJSON. DataFrame with columns ["id", "values"]
            columns=["id", "values"], # Must pass column 1 as the key, and column 2 the values.
            key_on="feature.id", # Variable in the geo_data GeoJSON file to bind the data to.
    ...
    def _create_popup_layer(self, popup_fields: List[str]) -> folium.GeoJson:
        ...
        # TODO: V2, Currently forced properties HeatMapConfig for display purposes
        self._log("⚠️ Popup layer settings (overlay, control) have been forced for consistent display.")
    ...
    def _add_static_map_info(self, m: folium.Map) -> None:
        ...
        if not raw_sources:
            self._log("ℹ️ No data source note specified in `HeatMapConfig`.\nYou can set one with `config.data_source_note = '...'`.")
